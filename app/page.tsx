...
async function getData() {
  ...
  const { data: quartiersData } = await supabase
    .from('properties')
    .select('quartier')
  const uniqueQuartiers = new Set(quartiersData?.map((p: any) => p.quartier)).size
  return { properties: properties ?? [], total: count ?? 0, quartiers: uniqueQuartiers }
}

export default async function HomePage() {
  const { properties, total, quartiers } = await getData()
        ...
        <div style={{position:'relative',zIndex:2,maxWidth:'1152px',margin:'0 auto',padding:'60px 24px',width:'100%'}}>
          <div style={{maxWidth:'540px'}}>
            ...
            <form action="/annonces" method="GET">
              <div style={{display:'flex',alignItems:'center',background:'var(--color-creme)',borderRadius:'12px',padding:'6px 6px 6px 14px',gap:'8px',maxWidth:'480px',boxShadow:'0 4px 24px rgba(0,0,0,0.15)'}}>
                <input name="q" type="text" placeholder="Quartier, type de logement…"
                  style={{flex:1,border:'none',background:'transparent',fontSize:'13px',fontFamily:'var(--font-dm)',color:'var(--color-marron)',outline:'none'}} />
            ...
            <div style={{display:'flex',gap:'32px',marginTop:'28px'}}>
              {[
                ...
                { val: quartiers > 0 ? quartiers : '—', label: 'Quartiers couverts' },
      ...
      <section style={{maxWidth:'1152px',margin:'0 auto',padding:'0 24px 80px'}}>
        <div style={{position:'relative',background:'var(--color-vert)',borderRadius:'16px',padding:'44px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'24px',overflow:'hidden'}}>
          ...
          <div style={{position:'relative',zIndex:1}}>
            <h2 style={{fontFamily:'var(--font-playfair)',fontSize:'24px',fontWeight:700,color:'var(--color-creme)',marginBottom:'8px'}}>
              Vous louez un logement ?
            </h2>
            <p style={{fontSize:'14px',color:'var(--color-vert-clair)',opacity:0.8,maxWidth:'380px'}}>
              Publiez gratuitement et touchez des milliers d'étudiants et de familles à Ziguinchor.
            </p>
          </div>
