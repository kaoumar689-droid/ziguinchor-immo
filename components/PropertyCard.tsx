...
export type Property = {
  ...
  titre: string
  prix: number
  type: 'chambre' | 'appartement' | 'maison'
  quartier: string
  chambres: number
  images?: string[]
  features?: string[]
...
const FEATURE_ICONS: Record<string, React.ReactNode> = {
  wifi: <Wifi size={12} />,
  electricite: <Zap size={12} />,
  eau: <Droplets size={12} />,
  parking: <Car size={12} />,
  sdb: <Bath size={12} />,
}
...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  return (
    <article style={{background:'#fff',borderRadius:'14px',border:'1px solid var(--color-creme-bord)',overflow:'hidden',transition:'transform 0.2s',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{padding:'14px',display:'flex',flexDirection:'column',gap:'6px',flex:1}}>
        ...
        <div style={{display:'flex',gap:'10px',fontSize:'11px',color:'var(--color-marron-doux)',borderTop:'1px solid var(--color-creme-bord)',paddingTop:'10px',marginTop:'4px',flexWrap:'wrap'}}>
          <span style={{display:'flex',alignItems:'center',gap:'3px'}}><BedDouble size={12} color="var(--color-vert)" />{property.chambres} ch.</span>
          {property.features?.slice(0,3).map(f=>(
            <span key={f} style={{display:'flex',alignItems:'center',gap:'3px'}}>
              <span style={{color:'var(--color-vert)'}}>{FEATURE_ICONS[f] ?? <Zap size={12}/>}</span>
